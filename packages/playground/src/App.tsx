...
import { Canvas, createT, Entity } from "solid-three"
...
// Vite glob imports
const apiModules = import.meta.glob("./api/**/*.tsx") as Record<
  string,
  () => Promise<{ default: any }>
>
...
const apiStructure = Object.keys(apiModules).reduce((acc, path) => {
  ...
  const category = parts[2] // api/category/file.tsx
  const fileName = parts[3]?.replace(".tsx", "")
  ...
  const router = (
    <Router root={Layout}>
      <For each={[...Object.entries(apiModules), ...Object.entries(exampleModules)]}>
        {([route, component]) => (
          <Route path={route.replace("./", "").replace(".tsx", "")} component={lazy(component)} />
        )}
      </For>
