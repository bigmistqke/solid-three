...
export function isAccessor<T>(value: AccessorMaybe<T>): value is Accessor<T> {
...
export async function awaitMapObject<T extends object, U>(
  object: T,
  callback: (value: T[keyof T], key: keyof T) => Promise<U> | U,
) {
  const result = {} as {
    [TKey in keyof T]: U
  }
...
export function defaultProps<T, TDefaults extends Partial<Required<Pick<T, KeyOfOptionals<T>>>>>(
  props: T,
  defaults: TDefaults,
) {
  return mergeProps(defaults, props) as Prettify<TDefaults & Omit<T, keyof TDefaults>>
}
...
export type LoadOutput<TLoader extends Loader<any, any>, TUrl> = TUrl extends Record<string, any>
  ? LoaderDataMap<TLoader, TUrl>
  : LoaderData<TLoader>

export async function load<
  const TLoader extends Loader<any, any>,
  TInput extends LoaderUrl<TLoader>,
>(loader: TLoader, input: TInput): Promise<LoaderData<TLoader>>

export async function load<
  const TLoader extends Loader<any, any>,
  TInput extends Record<string, LoaderData<TLoader>>,
>(loader: TLoader, input: TInput): Promise<LoaderDataMap<TLoader, TInput>>
...
export async function load(loader: Loader, input: string) {
  if (isRecord(input)) {
    return awaitMapObject(input, path => load(loader, path))
  }
